"""Stage/screen play text -> ProseCast IR (multi-voice, zero LLM).

Lifted out of scripts/play_to_ir.py (2026-09-04, E2.1) so the server can call
it directly; the script is now a thin CLI over this module and its flags,
output and stderr summary are unchanged.
...
Page breaks may be marked with \\f or not at all.

Known limit (surface it in the wizard's fine print): a dual-speaker label
("CHUCK & TILLY.") is treated as one character named "CHUCK & TILLY", not two.
"""
import difflib
import re
...
def parse(lines, running_head_re=None, keep_inline=False):
    """Text lines -> [{title, events:[(kind, speaker|None, text)]}].

    kind is 'speech' | 'direction' | 'narr'. ``keep_inline`` is accepted for
    call-compatibility with the original script; inline parens are handled at
    IR-build time, not here.
    """
    scenes = []
...
def build_ir(title, scenes, canon, keep_inline):
    """Scenes -> IR dict. Same shape ir_generator.build_ir produces."""
    def variants():
...
def play_to_ir(text, title, *, running_head="", keep_inline_parens=False,
               keep_front_matter=False) -> dict:
    """One call: play text -> {ir, scenes, blocks, dialogue, cast, merged}.

    ``text`` is the whole script as a string (or a list of lines).
    ``running_head`` is a regex; matching lines are dropped (title headers).
    """
    lines = text if isinstance(text, list) else text.split('\n')
    head_re = re.compile(running_head, re.I) if running_head else None
    scenes = parse(lines, head_re, keep_inline_parens)
    if not keep_front_matter and len(scenes) > 1 and scenes[0]["title"] == "Front Matter":
        scenes = scenes[1:]
    canon, merged = canonicalize_cast(scenes)
    ir = build_ir(title, scenes, canon, keep_inline_parens)
    return {
        "ir": ir,
        "scenes": len(ir["chapters"]),
        "blocks": sum(len(c["blocks"]) for c in ir["chapters"]),
        "dialogue": sum(1 for c in ir["chapters"] for b in c["blocks"]
                        if b["type"] == "dialogue"),
        "cast": ir["characters"],
        "merged": merged,
    }
