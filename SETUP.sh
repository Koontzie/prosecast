...
echo ""
echo "=== 9. Desktop shortcut (optional) ==="
...
make_shortcut_macos() {
  # A .command file cannot carry a custom icon, so the shortcut is a minimal
  # .app bundle that opens the .command in Terminal — one launch path, one
  # visible window, and closing it still stops the server.
  local app="$HERE/ProseCast.app"
  rm -rf "$app"
...
<plist version="1.0">
<dict>
  <key>CFBundleName</key>              <string>ProseCast</string>
  <key>CFBundleDisplayName</key>       <string>ProseCast</string>
  <key>CFBundleIdentifier</key>        <string>com.prosecast.launcher</string>
  <key>CFBundleExecutable</key>        <string>ProseCast</string>
...
PLIST_EOF
  cat > "$app/Contents/MacOS/ProseCast" <<APP_EOF
#!/bin/bash
# Generated by SETUP.sh. Hands the real launcher to Terminal so the server has
# a window someone can read and close.
open -a Terminal "$HERE/$LAUNCHER"
APP_EOF
  chmod 755 "$app/Contents/MacOS/ProseCast"
  cp icon.icns "$app/Contents/Resources/icon.icns"
  # Finder caches an app's icon by mtime; touching the bundle makes it notice.
  touch "$app"
  ok "built ProseCast.app"

  local link="$DESKTOP/ProseCast.app"
  if [ -e "$link" ] || [ -L "$link" ]; then
    if ask "$link already exists. Replace it? [y/N]" n; then
      rm -rf "$link"
    else
      ok "left the existing Desktop shortcut alone — ProseCast.app is in the ProseCast folder"
      return 0
    fi
  fi
  ln -s "$app" "$link"
...
make_shortcut_linux() {
  local dest="$DESKTOP" entry
  if [ ! -d "$dest" ]; then
    dest="$HOME/.local/share/applications"
    mkdir -p "$dest"
  fi
  entry="$dest/prosecast.desktop"
  if [ -e "$entry" ]; then
    if ! ask "$entry already exists. Replace it? [y/N]" n; then
      ok "left the existing shortcut alone"
      return 0
    fi
  fi
  cat > "$entry" <<DESKTOP_EOF
[Desktop Entry]
Type=Application
Name=ProseCast
Comment=Multi-voice audiobooks, made on your own machine
Exec=$HERE/$LAUNCHER
Icon=$HERE/icon.png
Path=$HERE
Terminal=true
...
DESKTOP_EOF
  chmod 755 "$entry"
  # GNOME shows an untrusted launcher as "Untrusted application launcher"
  # until this is set; on desktops without gio it is simply not needed.
...
if [ "$OS" = "Darwin" ]; then
  SHORTCUT_WHAT="a ProseCast.app in this folder, and a shortcut to it on your Desktop"
  SHORTCUT_NEEDS="icon.icns"
else
  SHORTCUT_WHAT="a prosecast.desktop launcher on your Desktop (or in your applications menu)"
  SHORTCUT_NEEDS="icon.png"
fi
if [ ! -f "$SHORTCUT_NEEDS" ]; then
  warn "no $SHORTCUT_NEEDS in this folder — skipping the shortcut. Rebuild the icons with: .venv/bin/python scripts/make_icons.py"
elif ask "Make a desktop shortcut? This creates $SHORTCUT_WHAT. [y/N]" n; then
  if [ "$OS" = "Darwin" ]; then make_shortcut_macos; else make_shortcut_linux; fi
else
  ok "no shortcut made — ./$LAUNCHER is the way in"
fi
