...
any more** — and nothing requires the README either. **E9** (09-06) closed the
eight findings from the first real Windows install, **E9.7** (09-07) polished
what the fresh-clone re-run found, and **E9.8** (09-07/08) is the second run's
six findings. **E10 (09-08)** is the parity chapter: `SETUP.sh` now reaches a
working voice engine, writes a launcher on macOS and Linux, and offers a
desktop shortcut on all three platforms — so the scripted install that ends in
audio and a file you double-click is no longer Windows-only. Next: the **cast
...
Findings (a) and (b) from the last session, closed. A Windows user got a
scripted install that ended in working audio and a file to double-click;
everyone else got a wizard whose step 2 could never go green and a uvicorn
command to type. **Tests 432 → 462 passed, 1 skipped. All five `tests/ui/`
checks green.** Five commits, none pushed.

**HANDOFF first.** It still said 375 tests and warned that E6, E7 and E9 were
sitting unpushed on the Mac; both had stopped being true. The phase table gains
E9.7 and E9.8, "Where to start" is rewritten around what is actually next, and
the key facts gain what the two Windows runs taught — streams are not files,
Piper as a module, the sample book's text-sha stamp, chapter 1's length as a
product decision, the two shipped gender tables, and conftest rebinding the
three import-time service URLs to a closed port.

**`SETUP.sh` reaches a working engine.** On Linux it installs `piper-tts` and
the six `PIPER_VOICES`, the same list `SETUP.ps1` installs and now held to it
by the same drift test. On macOS `say` already works, so Piper is *offered* —
"~400 MB — install now? [y/N]" — and never assumed. `bad` (records a ✗) is now
separate from `die` (exits), mirroring `SETUP.ps1`'s Fail/Die split, so one
voice that fails to download no longer ends the install.

**And it makes Piper prove it can speak.** This came out of testing rather than
planning. On an Apple Silicon Mac, `piper-tts` installs, the six voices
download, `probe_voice_engine` counts them and the Setup page goes **green** —
and every render writes a zero-byte wav, because the wheel bundles a
libespeak-ng whose data path is baked to the machine it was built on
(`/Users/runner/work/piper1-gpl/…`). `ESPEAK_DATA_PATH` does not rescue it and
neither does 1.7.0, 1.6.0 or 1.5.0; all four fail identically. Phonemizing
needs no voice model, so `piper_can_speak()` answers in a second and runs
**before** the 400 MB. A mute Piper is a `warn` on macOS (say still works) and
a `bad` on Linux (it is the only built-in engine there). Windows is unaffected
— E9's install rendered the sample on Piper.

**A launcher outside Windows.** `start-prosecast.command` on macOS,
`start-prosecast.sh` on Linux, generated by `SETUP.sh` the way `SETUP.ps1`
generates its own, `chmod 755`, gitignored. Sets `PYTHONUTF8`, opens the
browser three seconds in, and `exec`s uvicorn in the foreground so closing the
window stops the server. It names the venv's python rather than activating
anything — E9.8's rule, and the reason Piper is run as a module. The script
then ends the way `SETUP.ps1` does: prints the command, *then* offers to run it.

**An opt-in desktop shortcut, on all three.** Asked once, defaulting to no,
saying what it will create before it creates it, and never replacing an
existing shortcut without a second question. Windows: a `.lnk` via
`WScript.Shell` at `powershell.exe -File start-prosecast.ps1` with `icon.ico`
(powershell.exe rather than the .ps1, because double-clicking a .ps1 opens
Notepad). Linux: a `prosecast.desktop` in `~/Desktop`, or
`~/.local/share/applications` when there is no Desktop. macOS: a minimal
`ProseCast.app` — Info.plist, `MacOS/` launch script, `Resources/icon.icns` —
with a Desktop symlink, because a `.command` file cannot carry a custom icon.
The bundle does not start a second server; it hands the `.command` to Terminal,
so there is one launch path and a window you can read and close.

**The mark.** `docs/assets/prosecast-icon.svg` is the source of truth: a
broadcast microphone with its colours lifted verbatim from the `--oa-*` tokens
in `static/index.html` rather than picked to look similar. Four solid shapes,
no stroke thinner than 30 units at 512, no text — it is seen at 16px more than
anywhere else. `scripts/make_icons.py` renders `icon.png` (512), `icon.ico`
(six sizes) and `icon.icns` (16–1024 with @2x) through Playwright's chromium,
which the `tests/ui/` checks already require, rather than adding rsvg or Pillow
to `requirements.txt` for a file that changes once a year. The `.ico` is
written by hand — an ICO directory may point straight at PNG data.

**`tests/test_setup_scripts.py`** holds the two scripts to the same shape.
Most of it is static, because `SETUP.ps1` cannot be run here. But `ask()` — the
one function between the script and a 400 MB download, someone's Desktop, or a
running server — is lifted out of the file and driven through a **real pty**:
Enter takes the stated default, only `y` is consent, and no terminal at all is
always no, whatever the default says.

**How this was actually verified**, since "linted by eye" is what E9 had to
say: the repo was cloned into a temp directory and `SETUP.sh` run in it under
a pty, twice. First run, answering yes to Piper: six steps green, six voices
downloaded, smoke test passed, `start-prosecast.command` written, exit 0. The
launcher was then started with a stubbed `open` on PATH — `GET /` returned
**200**, `/setup/status` reported the wizard-firing state, and the browser
opener fired on the right URL. Second run, with `HOME` pointed at a scratch
directory so nothing touched a real Desktop: the mute-Piper branch fired and
skipped the download, the `.app` bundle built, `plutil -lint` passed, and the
Desktop symlink landed.

**Still Tyler's to verify — neither is checkable from this Mac:**

- **The Windows shortcut and its `.lnk` icon**, on the laptop. Step 9 of
  `SETUP.ps1` has never been executed; there is no PowerShell here.
- **A clean Mac install on a machine that has never seen the repo** — the temp
  clone shared this machine's Homebrew ffmpeg and tesseract, so "external
  tools" was green for reasons a stranger would not have.

Also worth a look, and not urgent: whether the `ProseCast.app` icon actually
renders in Finder and the Dock. It is built and structurally checked, never
double-clicked.
